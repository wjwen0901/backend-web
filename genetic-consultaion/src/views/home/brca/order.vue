<template>
  <div>
    <div class="user-container">
          ...
          <el-row :gutter="0">
            ...
            <el-col :span="6">
              <el-form-item label="医生">
                <el-input v-model="searchForm.doctor" placeholder="王大夫、李大夫（顿号分隔多选）" clearable @change="handleSearch"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
          ...
          <el-row :gutter="0">
            <el-col :span="6">
              <el-form-item label="实验室">
                <el-select class="width-100-p" v-model="searchForm.companyId" clearable placeholder="全部实验室" @change="handleSearch">
      ...
      <div class="opera-box">
        <el-button type="primary" size="small" :disabled="!hasSelection" @click="toDistribution">分配解读费用</el-button>
        <el-button type="primary" size="small" :disabled="!hasSelection" @click="toDistributionSalesman">分配市场费用</el-button>
        <el-button type="success" size="small" :disabled="!hasSelection" @click="confirmMoney">确认到账</el-button>
        <el-button type="success" size="small" :disabled="!hasSelection" @click="exportOrder">导出选中</el-button>
      ...
      <el-table
        ...
        <el-table-column prop="createTime" label="下单时间" width="136" fixed="left">
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="text" @click="toUploadReport(scope.row.orderId)" v-if="canUploadReport(scope.row.status)">报告</el-button>
          </template>
        </el-table-column>
    ...
    <transition name="bulk-bar">
      <div class="bulk-bar" v-show="hasSelection">
        ...
        <el-button type="primary" size="small" @click="toDistribution">分配解读</el-button>
        <el-button type="primary" size="small" @click="toDistributionSalesman">分配市场</el-button>
        <el-button type="success" size="small" @click="confirmMoney">确认到账</el-button>
        <el-button type="success" size="small" @click="exportOrder">导出</el-button>
        <el-button size="small" plain @click="clearSelection">取消选中</el-button>
      </div>
    </transition>

    <el-dialog title="解读费用分账" :visible.sync="dialogDistributionVisible" width="640px">
      <div class="callout">
        <h5>操作说明</h5>
        <ol>
          <li>仅支持选择「同一位医生」的订单进行分账</li>
          <li>勾选订单后从下拉里选择该医生的服务人员</li>
          <li>提交后将按该医生当前规则结算</li>
        </ol>
      </div>
      <el-form ref="distributionForm" :model="distribution" label-width="120px" size="small">
        <el-form-item label="相关医生">
          <span class="readonly">{{ distribution.doctorNames || '—' }}</span>
        </el-form-item>
        <el-form-item label="选择服务人员">
          <el-select v-model="distribution.userId" filterable placeholder="请选择" class="width-100-p">
            <el-option v-for="item in serviceUserList" :key="item.userId" :label="item.fullName" :value="item.userId"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button @click="dialogDistributionVisible = false">取消</el-button>
        <el-button type="primary" @click="submitDistribution(1)" :disabled="!distribution.userId">确定分账</el-button>
      </span>
    </el-dialog>

    <el-dialog title="市场费用分配" :visible.sync="dialogDistributionSalesVisible" width="640px">
      <div class="callout">
        <h5>操作说明</h5>
        <ol>
          <li>仅支持选择「同一位医生」的订单进行市场费用分配</li>
          <li>勾选订单后从下拉里选择该医生的业务代表</li>
        </ol>
      </div>
      <el-form ref="distributionSalesForm" :model="distribution" label-width="120px" size="small">
        <el-form-item label="相关医生">
          <span class="readonly">{{ distribution.doctorNames || '—' }}</span>
        </el-form-item>
        <el-form-item label="选择业务代表">
          <el-select v-model="distribution.userId" filterable placeholder="请选择" class="width-100-p">
            <el-option v-for="item in serviceUserList" :key="item.userId" :label="item.fullName" :value="item.userId"></el-option>
          </el-select>
        </el-form-item>
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="submitDistribution(0)" :disabled="!distribution.userId">确定分配</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      orderList: [],
      searchForm: {},
      distribution: { doctorNames: '', orderIds: [], userId: '' },
  ...
  computed: {
    ...
    emptyHint () {
      return this.hasFilter ? '试试调整筛选条件，或清空后重新查询' : '暂无 BRCA 订单数据'
    },
    hasFilter () {
      const f = this.searchForm
      return !!(f.hospitalId || f.deptId || f.doctor || f.patient || f.companyId || f.status || f.startDate || f.endDate)
    }
  },
  filters: { formatDate },
  methods: {
    ...
    _collectDoctors (serviceType) {
      const doctorIds = []
      this.distribution = { doctorNames: '', orderIds: [], userId: '', serviceType: serviceType }
      ...
      this.multipleSelection.forEach(item => {
        ...
        if (doctorIds.indexOf(item.doctorId) === -1) {
          ...
          this.distribution.doctorNames = this.distribution.doctorNames
            ? this.distribution.doctorNames + '  ' + item.doctorName
            : item.doctorName
        }
      })
      const url = serviceType === 1 ? 'doctor/service' : 'doctor/salesman'
      doctorIds.forEach(id => {
        this.axios.get(url, { params: { doctorId: id } }).then(res => {
          if (res.data.length) {
    ...
    exportOrder () {
      const ids = this.multipleSelection.map(i => i.orderId).join(',')
